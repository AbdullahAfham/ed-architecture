...
class ResUsers(models.Model):
    ...
    status = fields.Selection(string='Mobile Status', selection=[('unverified', 'Unverified'), ('verified', 'Verified')],
                              default='unverified')
    profile_unique = fields.Char('User Profile unique')
    default_partner_id = fields.Many2one('res.partner', string='Default Shipping Address')
    device_id = fields.Many2one('device.info', string='Device')
    ...
    restrict_device = fields.Boolean(string='Restrict Device', default=True)
    mobile_module_ids = fields.Many2many(
        comodel_name='mobile.module',
        string="Modules",
        context={'active_test': False},
        help="Modules that apply on the mobile app"
    )
    allow_warehouse_ids = fields.Many2many(
        comodel_name='stock.warehouse',
        string="Allowed Warehouses",
        domain="[('company_id', 'in', company_ids)]",
    )
    allow_journal_ids = fields.Many2many(
    ...
    def _create_user_from_template(self, values):
        ...
        # print("++++++++++++++++++++++++++++++template_uesr", template_user)
